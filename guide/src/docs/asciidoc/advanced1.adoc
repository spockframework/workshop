...
Reference documentation http://spockframework.org/spock/docs/2.0/extensions.html#_writing_custom_extensions[Spock - Writing Custom Extensions]
...
To do something when the actual feature is executed you have to register an http://spockframework.org/spock/docs/2.0/extensions.html#_interceptors[interceptor] at the appropriate location.

.Spock Interceptors
image:http://spockframework.org/spock/docs/2.0/images/spock_interceptors.png[]
...
@Override
public void visitSpec(SpecInfo spec) {
    if (Boolean.parseBoolean(System.getProperty("ExecutionLogExtension.enabled", "true"))) {
        spec.addInterceptor(interceptor);
        spec.getAllFeatures().forEach(featureInfo -> featureInfo.addIterationInterceptor(interceptor));
    }
}
...
=== Step 6
While a system property certainly works, a more powerful way is to use http://spockframework.org/spock/docs/2.0/extensions.html#_configuration_objects[Configuration Objects].
...
=== Step 7
Add a `SpockConfig.groovy` to the root of `src/test/resources` and configure the extension.
